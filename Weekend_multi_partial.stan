// Formula: profit_pct ~ 1 
//          sigma ~ 1 + (1 | ticker) + Momentum_60 + IVz + VVIXz
...
model {
  alpha ~ normal(0,1)  ;
  xi_I ~ normal(0, 1);
  omega_I_mu ~ normal(0, 1);
  omega_I_sigma ~ exponential(1);
  omega_I ~ normal(omega_I_mu, omega_I_sigma);
  omega_B ~  normal(0, 1);
