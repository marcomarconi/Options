...
# Load single file assets prices (Yahoo) 
{
    vol_shift <- 21; period <- 252
...
# Loading Barchart historical single option data (the ones individually scraped from pages like https://www.barchart.com/stocks/quotes/SPY%7C20200814%7C321.00C/price-history/historical)
{
...
# Load Dolthub volatility data single file (BEWARE, only 3 days per week, VRPlog might be different)
{
...
# ORATS core data loading
{
    ...
    load_orats_day <- function(filename, cols_to_extract) {
        quiet_read_csv(glue::glue("/home/marco/ORATS/cores/{filename}")) %>%
        #quiet_fread(glue::glue("/home/marco/ORATS/cores/{filename}")) %>% 
            purrr::pluck("result")  %>%   select(-contains(c("dlt", "ern", "div", "fw", "Fit"), ignore.case = TRUE)) #%>%   select(all_of(cols_to_extract))
    }
    
    cols_to_extract <- c('ticker', 'tradeDate', 'assetType', 'pxAtmIv', 'mktCap', 'iv30d', 'clsHv20d', 'avgOptVolu20d', 'sector', 
                         'sectorName', 'mktCap', 'beta1y')
    
    dir <- "/home/marco/ORATS/cores"
    files <- list.files(dir, pattern = "2024.*gz")
    core_df <- files %>% purrr::map_df(.f = load_orats_day, cols_to_extract)
...
# Plot some general observations from barchart data 
{
    ...
    # Stocks seem to be slightly mean-reverting
    stocks %>% mutate(Decile = ntile(ReturnPrice %>% na.locf0() %>% SMA(20) %>% lag, 5)) %>% group_by(Symbol, Decile, Year=year(Date)) %>% reframe(VRP=median(ReturnPrice, na.rm=T))  %>% group_by(Decile, Year) %>% reframe(M=mean(VRP, na.rm=T), S=sd(VRP, na.rm=T)/sqrt(n())*2, N=n()) %>% na.omit %>% ggplot(aes(x=Decile, ymin=M-S, ymax=M+S)) + geom_errorbar() + facet_wrap(~Year, scales = "free")    
...
# Single stock playing with Dolthub (SPY as example)
{
    ticker <- "SPY"
...
# Single stock playing with ORATS (SPY as example)
{
    f <- read_csv("/home/marco/ORATS/strikes/IWM/IWM_.csv")
    df_file <- f %>% mutate(delta_ = abs(round_to_nearest(delta, 0.1)), dist = 1/(abs(dte - 30)+1))
    ...
    # Straddles over weekend analysis
    ...
    ff %>% filter(delta_==0.5 & dte <= 7) %>% group_by(wd) %>% reframe(M=mean(VRPlog, na.rm=T), S=2*sd(VRPlog, na.rm=T)/sqrt(n()), N=n())
    # Straddles over weekend backtest
